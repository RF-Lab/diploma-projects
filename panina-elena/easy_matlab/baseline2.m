function [omc,bas] = baseline2(X_i,obs1,obs2,sats,time,Eph,t_delta1, t_delta2)
...
for iter = 1:8
    ...
    [tcorr,rhok_j,Xk_ECF] = get_rho(time+t_delta2, obs2(1), Eph(:,col_Eph(1)),X_j);
    %t_delta2 = t_delta2 + tcorr ;
    [tcorr,rhok_i,Xk_ECF] = get_rho(time+t_delta1, obs1(1), Eph(:,col_Eph(1)),X_i);    
    %t_delta1 = t_delta1 + tcorr ;
    for t = 2:m % t runs over PRNs given in sats; ref.sat. is number 1
        [tcorr,rhol_j,Xl_ECF] = get_rho(time+t_delta2, obs2(t), Eph(:,col_Eph(t)), X_j);
        %t_delta2 = t_delta2 + tcorr ;
        [tcorr,rhol_i,Xl_ECF] = get_rho(time+t_delta1, obs1(t), Eph(:,col_Eph(t)), X_i); 
        %t_delta1 = t_delta1 + tcorr ;
